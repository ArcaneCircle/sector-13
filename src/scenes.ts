...
function commsText(text: string, i: number) {
  return getText(
    text,
    commsSprite.width / SCALE / 2,
    commsSprite.y / SCALE + 9 * (i + 1),
    { anchor: { x: 0.5, y: 0.5 } },
  );
}
...
function initScenes() {
  ...
  data.scenes.fear.add(getText("fear", 4, 257), fearSprite, fearSpriteInner);
  ...
  data.scenes.communication.onShow = () => {
    ...
    data.scenes.communication.add(
      ...
      getText(
        ...
        commsSprite.width / SCALE / 2,
        commsSprite.y / SCALE + commsSprite.height / SCALE - 8,
        { anchor: { x: 0.5, y: 0.5 } },
      ),
    );
  ...
  const highscore = () => [
    ...
    Sprite({ ...getScore(window.highscores.getScore()), x: 560, y: 1250 }),
  ];

  data.buttons.story = getBoldText(data.labels.story, 59, 149, {
    button: true,
  });
  data.buttons.hardcore = getBoldText(data.labels.hardcore, 48, 189, {
    button: true,
  });
  ...
  const textSprites = [
    ...arr.map(([x, y], i) =>
      clearUnreached(
        getText(data.labels.sector, x, y, { sectorButton: true }),
        i,
      ),
    ),
  ];
  const numberSprites = [
    ...arr.map(([x, y], i) =>
      clearUnreached(getNumbers((i + 1).toString(), x + 8, y + 6), i),
    ),
  ];
  ...
  data.buttons.restart = getBoldText(data.labels.restart, 52, 191, {
    button: true,
  });
  data.scenes.end.add(
    ...(() => [
  ...
  data.scenes.end.onShow = () => {
    if (state.lives < 0) {
      data.scenes.end.add(
        getBoldText(data.labels.loser, 28, 86, { scale: 18 }),
      );
    } else {
      data.scenes.end.add(
        getBoldText(data.labels.winner, 32, 86, { scale: 19 }),
      );
    }
    data.scenes.end.add([
      ...highscore(),
      Sprite({ ...getScore(state.score), x: 560, y: 1570 }),
    ]);
  ...
  class Star extends SpriteClass {
    constructor(properties: any = {}) {
      ...
      this.width = 10;
      this.height = 10;
      this.opacity = randInt(25, 75) * 0.01;
      this.color = "#fff";
