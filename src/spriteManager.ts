...
const powerupManager = new Manager((powerup) => {
...
const playerShieldManager = new Manager((shield) => {
  ...
  update() {
    this.assets.forEach((bomb) => {
      ...
      if (!target) {
        ...
        this.targets.set(bomb, {
          target: enemy,
          isForward: state.playerY < enemy.y,
        });

        target = enemy;
      }

      let lerpNum = isForward ? 0.1 : 0.025;
...
const scoreDisplayManager = new Manager(
  (sprite) => (sprite.opacity = sprite.opacity -= 0.05),
);
const scoreMultDisplayManager = new Manager(
  (sprite) => (sprite.opacity = sprite.opacity -= 0.01),
);

const explosionManager = new Manager((explosion) => {
  explosion.animations.explode.isStopped
    ? (explosion.opacity = 0)
